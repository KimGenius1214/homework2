...
<script>
	function replyDelete(r_idx){
		...
		if(yn){location.href='./reply_delete.jsp?r_idx='+r_idx+'&b_idx='+'<%=b_idx%>';
		}
	}
...
%>
    <p><%= r_user %> : <%= r_content %> (<%= r_regdate %>)
    
<% 
     if(r_user.equals((String)session.getAttribute("userid"))){
%>
            
     <input type="button" value="댓글삭제"  onclick="replyDelete('<%=r_idx%>')">
<%
