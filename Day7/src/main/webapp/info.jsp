...
<%
	...
	if((member = dao.info(Integer.parseInt(String.valueOf(session.getAttribute("idx"))))) == null){
...
<body>
    ...
    <form action="./info_ok.jsp" method="post" name="regform" id="regform" onsubmit="return sendit()">
        <p>아이디 : <%=(String)session.getAttribute("userid") %></p>
        ...
        <p>취미 : 
            ...
            <% 
            if(compare_hobby("게임", member.getHobby())){
        		out.print("checked");
        	}
        	%>
        	></label>
            ...
            <% 
            if(compare_hobby("운동", member.getHobby())){
        		out.print("checked");
        	}
        	%>
        	></label>
        </p>
        <p>주민등록번호 : <%=member.getSsn1() %> - <%=member.getSsn2() %></p>
        ...
        <p>상세주소 : <input type="text" name="address2" id="sample6_detailAddress" value="<%=member.getAddress2() %>"></p>
